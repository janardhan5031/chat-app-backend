...
function getAllUsers(){
    ...
    axios.get(`http://13.127.49.91/user/getAllUsers`,{headers:{"authorization":token}})
    .then(async (result) =>{
        console.log(result);
...
async function getAllGroups(){
    ...
    axios.get(`http://13.127.49.91/group/get-all`,{headers:{"authorization":token}})
    .then(result =>{
...
async function getMsgsFromBackend(){
    ...
    await axios.get(`http://13.127.49.91/chat/${path}?send_to=${recieving_person_id}&last_msg_id=${last_msg_id}`,{headers:{"authorization":token}})
    .then(async(result)=>{
...
document.getElementById('send_btn').addEventListener('click',(e)=>{
    ...
    if(input_field.type === 'file'){
        ...
        console.log(file)
        axios.post(`http://13.127.49.91/${path}/file?send_to=${send_to}`,file,{headers:{"authorization":token}})
        .then(result =>{
            console.log(result)
        })
        .catch(err => console.log(err));
    ...
    // if the input field is text type, store msg in database 
        ...
        axios.post(`http://13.127.49.91/${path}?send_to=${send_to}`,{msg},{headers:{"authorization":token}})
        .then(result=>{
            console.log(result);
...
document.getElementById('add_group_name').addEventListener('click',(e) =>{
    ...
    axios.post(`http://13.127.49.91/group/create-group`,{groupName},{headers:{"authorization":token}})
    .then(result=>{
        console.log(result);
...
async function update_group_in_backend(key, userId){
    ...
    await axios.post(`http://13.127.49.91/group/${key}`,{groupId,userId},{headers:{"authorization":token}})
    .then((result)=>{
        //console.log(result)
        window.alert(result.data.msg)
    })
